...
class ATM {
  ...
  /// Returns Map<int,int>, which uses bills types as a keys
  /// and number of bills as a value.
  Map<int, int> getCash(int cash) {
    var bills = this.getBills();

    // Throws an [ArgumentError] if there is an error with input parameters.
    if (bills.isEmpty) {
    ...
    for (int i = bills.length - 1; i >= 0; i--) {
      // Breaks cycle if current best parameter is better then 
      // it can be in current iteration
      ...
      if (cash >= curBill) {
        ...
        var paramList =
            this.getBestMapOfBills(i, cash - curBill, bestParam, curParam);
        mapOfBills[curBill]--;
    ...
    // Throws an [ArgumentError] if there is no possibility to give cash
    // with available bills.
  ...
  ParametersOfAlgorithm getBestMapOfBills(int index, int cash,
    ...
    // Exits from the recursion if current best number of 
    // different bills more then max possible in current depth of recursion
    ...
    // Continues recursiveness if cash > 0
    // Calls recursion for current bill if cash >= curBill
    // Calls recursion for the next bill if 0 < cash < curBill
    if (cash >= curBill) {
      for (int i = index; i >= 0; i--) {
        ...
        var newParam =
            this.getBestMapOfBills(i, cash - tempBill, bestParam, curParam);
