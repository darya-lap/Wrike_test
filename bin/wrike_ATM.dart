...
class ATM {
  Map getCash(int cash, List<int> bills_param) {
    ...
    final mapOfBills = new Map.fromIterable(
        List.generate(bills.length, (int index) => bills_param[index]),
        value: (_) => 0);
    ...
    for (int i = bills.length - 1; i >= 0; i--) {
      ...
      if (cash_rest >= bills[i]) {
        ...
        var paramList = this.getBestMapOfBills(
            bills, i, cash_rest - bills[i], bestParam, curParam);
        mapOfBills[bills[i]]--;
  ...
  ParametersOfAlgorithm getBestMapOfBills(List<int> bills, int index, int cash,
    ...
    if (cash >= curBill) {
      for (int i = index; i >= 0; i--) {
        ...
        var newParam = this
            .getBestMapOfBills(bills, i, cash - bills[i], bestParam, curParam);
      ...
      if (cash > 0) {
        for (int i = index - 1; i >= 0; i--) {
          if (cash >= bills[i]) {
            curParam.incrementNumberOfDiffBills();
            curParam.incrementNumberOfBillByKey(bills[i]);
            var newParam = this.getBestMapOfBills(
                bills, i, cash - bills[i], bestParam, curParam);
            if (newParam.getNumberOfDiffBills() >
                bestParam.getNumberOfDiffBills()) {
              bestParam = newParam;
            }
